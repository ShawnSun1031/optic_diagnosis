...
mode = ["ground","train"]
sim_index=1:1; % how many simulation are there to check
num_SDS=7; % how many SDS are there
...
for m=1:size(mode,2)
    one_conf=load(fullfile(input_folder, mode(m),'sim_1','cfg_1.mat'));
    ...
    if use_par_mode==0
        ...
        for i=sim_index
            ...
            try
                temp_sim_sum=jsondecode(fileread(fullfile(input_folder, mode(m),['sim_' num2str(i)],'sim_summary_1.json')));
            catch
                assert(false,['ERROR: sim ' num2str(i) ' is not completed! sim summary not exist!']);
            end
            try
                temp_PL=load(fullfile(input_folder, mode(m),['sim_' num2str(i)],'PL_1.mat'));
            catch
                assert(false,['ERROR: sim ' num2str(i) ' is not completed! PL.mat not exist!']);
            end
            try
                temp_conf=load(fullfile(input_folder, mode(m),['sim_' num2str(i)],'cfg_1.mat'));
            catch
            ...
            assert(temp_sim_sum.num_photon==num_photon(m),['ERROR: the photon number of sim ' num2str(i) ' is wrong!']);
            for s=1:num_SDS
