...
radius_w = 0.05;
h = [0, 0.1, 0.2, 0.4]; detector_deltr = 0.01; FOV = pi*1/6; D = 2*detector_deltr; L = 0; thetad = 0.5*FOV;
Rrtheta = zeros(4,10,24);
for epoch = 1:4
    ...
    for i = 1:N
        x = radius_w*sqrt(-log(1-rand(1))/2); y = 0; z = 0;
        cx = 0; cy = 0; cz = 1;w=1;
        xx = x; yy = y; zz = z; zz = z;
        ...
        while ~absorb & in_tissue
            ...
            if (0<z) & (z<(d+d2)) | is_reflect | is_layer         %%%%% in the tissue
                ...
                elseif  d<= zz & zz<(d+d2) & 0<=z & z<d & ~is_layer  %%% layer2 to layer1
                    ...
                    flag = false;
                    if cz>0
                        cz = -cz;      %%%%%%%%%  layer2 --> layer1 --> out ---> layer1 
                        flag = true;
                        temp = z;    
                    end
                    x = xx + cx*(s1+s2);
    ...
    %                     r = 1/2*(sin(incident_angle-thetaT)^2/sin(incident_angle+thetaT)^2+tan(incident_angle-thetaT)^2/tan(incident_angle+thetaT)^2);
                    ...
                    ir = sqrt(x^2+y^2);
                    ir = ceil(ir/detector_deltr-0.5)+1;
                    itheta = theta1;
                    itheta = ceil(itheta/(pi/24)-0.5)+1;
                    if theta1 <= pi*0.5
                        if 1<= ir & ir<= 10*(100*detector_deltr) & 1 <= itheta & itheta <= 24
                            Rrtheta(epoch,ir,itheta) = Rrtheta(epoch,ir,itheta) + tw;
                        end
                    end
...
% plot(Rrtheta)
% L = 10*r, D=5*r, FOV=15,30,60,180, h=0;
x = zeros(4,1,4);
for i=1:4
    x(i,1,1) = Rrtheta(i,10,1) + Rrtheta(i,9,1) + Rrtheta(i,8,1);
    x(i,1,2) = Rrtheta(i,10,2) + Rrtheta(i,9,2) + Rrtheta(i,8,2) + x(i,1,1);
    x(i,1,3) = Rrtheta(i,10,4) + Rrtheta(i,9,4) + Rrtheta(i,8,4) + x(i,1,1) +Rrtheta(i,10,3) + Rrtheta(i,9,3) + Rrtheta(i,8,3);
    x(i,1,4) = sum(Rrtheta(i,10,:)) +  sum(Rrtheta(i,9,:)) + sum(Rrtheta(i,8,:));
end
h0 = [x(1,1,1),x(1,1,2),x(1,1,3),x(1,1,4)];
h1 = [x(2,1,1),x(2,1,2),x(2,1,3),x(2,1,4)];
h2 = [x(3,1,1),x(3,1,2),x(3,1,3),x(3,1,4)];
h3 = [x(4,1,1),x(4,1,2),x(4,1,3),x(4,1,4)];
plot([15,30,60,180],h0,'-*r',[15,30,60,180],h1,[15,30,60,180],h2,[15,30,60,180],h3)
xlabel('detector diameter D')
ylabel('# of detect photons')
legend('h=0cm','h=0.1cm','h=0.2cm','h=0.4cm')
title('L = 10*r, D=5*r, plot with FOV=15,30,60,180, h=0')
