...
radius_w = 0.05;
h = 0; detector_deltr = 0.01; FOV = pi*1/6; D = 2*detector_deltr; L = 0; thetad = 0.5*FOV;
for epoch = 1:5
    Rrtheta = zeros(10,24);
    Arz = zeros(wide/deltr,deep/deltz);
    Av = zeros(wide/deltr,deep/deltz);
    ...
    %                     r = 1/2*(sin(incident_angle-thetaT)^2/sin(incident_angle+thetaT)^2+tan(incident_angle-thetaT)^2/tan(incident_angle+thetaT)^2);
                    ...
                    if theta1 <= pi*0.5
                        if 1<= ir & ir<= 10*(100*detector_deltr) & 1 <= itheta & itheta <= 24
                            Rrtheta(ir,itheta) = Rrtheta(ir,itheta) + tw;
                        end
                    end
...
% plot(Rrtheta)
%%%%%(a) FOV=30,(¬Ý15,,30/2=15),D=2*r;
x = zeros(1,5);
x(1) = Rrtheta(1,2) + Rrtheta(1,1);
x(2) = Rrtheta(1,2)+Rrtheta(2,2)+Rrtheta(1,1)+Rrtheta(2,1);
x(3) = Rrtheta(2,2)+Rrtheta(3,2)+Rrtheta(2,1)+Rrtheta(3,1);
x(4) = Rrtheta(4,2)+Rrtheta(5,2)+Rrtheta(4,1)+Rrtheta(5,1);
x(5) = Rrtheta(10,2)+Rrtheta(10,1);
plot([0,1,2,5,10],x,'-*b')
xlabel('distance L between the center of the detector and the incident ray')
ylabel('# of detect photons')
title('h=0,D=2*r,FOV=30,plot with L=0,r,2r,5r,10r')
...
x(1) = Rrtheta(1,2)+ Rrtheta(1,1);
x(2) = Rrtheta(1,2)+Rrtheta(2,2)+Rrtheta(1,1)+Rrtheta(2,1);
x(3) = Rrtheta(1,2) + Rrtheta(2,2) + Rrtheta(3,2) + Rrtheta(4,2) + Rrtheta(5,2)+Rrtheta(1,1) + Rrtheta(2,1) + Rrtheta(3,1) + Rrtheta(4,1) + Rrtheta(5,1);
x(4) = sum(Rrtheta(:,2))+sum(Rrtheta(:,1));
plot([1,2,5,10],x,'-*r')
xlabel('detector diameter D')
ylabel('# of detect photons')
title('L=0,h=0,FOV=30,plot with D=r,2r,5r,10r')
