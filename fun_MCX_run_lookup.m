...
function fun_MCX_run_lookup(target_name,subject)
...
model_folder='models';
MRI_voxel_file=fullfile(model_folder,'headModel'+ subject+ '_EEG.mat');
load(MRI_voxel_file);
...
p_pos=load(fullfile(target_name,[subject '_probe_pos.txt']));
p_dir=load(fullfile(target_name,[subject '_probe_dir.txt']));
if model_version>=2
...
while now_sim_index<=sim_index_set(2)
    ...
    if simed_index_arr(now_sim_index)==0
        ...
        fun_MCX_sim_dist2axis(temp_param,vol,voxel_size,p_pos,p_dir,sim_set,output_folder,0,1,0,1,0.005,10000);
